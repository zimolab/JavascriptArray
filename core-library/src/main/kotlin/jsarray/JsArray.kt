...
open class JsArray(val source: JSObject) {
    ...
    companion object {
        private const val CHECK_IS_JS_ARRAY = "Object.prototype.toString.apply(%s) === '[object Array]'"
    ...
    open fun concat(other: JsArray): JsArray {
    ...
    open fun concat(other: JSObject): JsArray {
    ...
    open fun push(vararg elements: Any?): Int {
    ...
    open fun unshift(vararg elements: Any?): Int {
    ...
    open fun slice(start: Int, end: Int? = null): JsArray {
    ...
    open fun splice(index: Int, count: Int, vararg items: Any?): JsArray {
    ...
    open fun fill(value: Any?, start: Int = 0, end: Int? = null): JsArray {
        val result = if (end == null) {
    ...
    open fun find(callback: JsArrayFindCallback): Any? {
    ...
    open fun findIndex(callback: JsArrayFindCallback): Int {
    ...
    open fun includes(ele: Any?, start: Int = 0): Boolean {
        return source.call("includes", ele, start) == true
    }

    open fun indexOf(ele: Any?, start: Int = 0): Int {
    ...
    open fun lastIndexOf(ele: Any?, start: Int = -1): Int {
    ...
    // 以下迭代方法
    open fun forEach(callback: (Int, Any?) -> Unit) {
    ...
    open fun jsForEach(callback: JsArrayForEachCallback) {
        inject(source, "forEach", callback) { source, method->
            source.eval("this.forEach((item, index, arr)=>{ ${method}(index, item) })")
        }
    }

    open fun jsFor(callback: JsArrayForCallback, startIndex: Int = 0, stopIndex: Int = -1, step: Int = 1) {
    ...
    open fun filter(block: (currentItem: Any?, index: Int, arr: JsArray)->Boolean): JsArray {
        val range = (0 until length)
    ...
    open fun jsFilter(callback: JsArrayFilterCallback): JsArray {
    ...
    open fun map(callback: JsArrayMapCallback): JsArray {
    ...
    open fun every(callback: JsArrayEveryCallback): Boolean {
    ...
    open fun some(callback: JsArrayEveryCallback): Boolean {
    ...
    open fun reduce(callback: JsArrayReduceCallback): Any? {
    ...
    open fun reduceRight(callback: JsArrayReduceCallback): Any? {
